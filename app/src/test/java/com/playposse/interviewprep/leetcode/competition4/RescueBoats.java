...
public class RescueBoats {
    ...
    class Solution {
        public int numRescueBoats(int[] people, int limit) {
            ...
            Collections.sort(peopleList, Collections.<Integer>reverseOrder());

            int boatCount = 0;
            int secondIndex = 0;
            while (!peopleList.isEmpty()) {
                boatCount++;
                int bigPerson = peopleList.remove(0);

                if (!peopleList.isEmpty()) {
                    int complement = limit - bigPerson;

                    while (secondIndex >= peopleList.size()) {
                        secondIndex--;
                    }

                    while ((secondIndex > 0) && (peopleList.get(secondIndex) < complement)) {
                        secondIndex--;
                    }

                    while ((secondIndex < peopleList.size() - 1)
                            && (peopleList.get(secondIndex) > complement)) {
                        secondIndex++;
                    }

                    if (peopleList.get(secondIndex) <= complement) {
                        peopleList.remove(secondIndex);
                    }

//                    for (int i = 0; i < peopleList.size(); i++) {
//                        if (peopleList.get(i) <= complement) {
//                            peopleList.remove(i);
//                            break;
//                        }
//                    }
